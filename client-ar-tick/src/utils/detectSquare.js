...
const computeGlobalThreshold = (cv, warped, boxes) => {
  ...
  const t = cv.threshold(gray, thresh, 0, 255, cv.THRESH_BINARY_INV + cv.THRESH_OTSU);
  ...
  try {
      ...
      try {
        ...
        results.forEach((result) => {
          ...
          cv.putText(
            debugMat,
            label,
            new cv.Point(checkbox.position.x + checkbox.position.size + 10, 
    ...
    if (checkedBoxes.length > 0 && qrId) {
      try {
        ...
        console.log("📤 Sending checked boxes:", JSON.stringify(payload, null, 2));
