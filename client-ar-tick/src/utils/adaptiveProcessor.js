...
export class AdaptiveProcessor {
  ...
  processResults(results, isDynamicMode = true) {
    ...
    for (const result of results) {
      ...
      if (result.confidence < this.minConfidence) {
        if (this.debugMode) {
          console.log(
            `⚠️ Skipping #${key} - low confidence: ${result.confidence}`,
          );
        }
        continue;
      }
    ...
    if (this.debugMode && adjustments.length > 0) {
      console.log(
        `🔄 Adaptive processor suggested ${adjustments.length} adjustment(s)`,
      );
    }
  ...
  suggestAdjustment(history, key) {
    ...
    const needsAdjustment = this.needsAdjustment(
      avgFill,
      variance,
      fillTrend,
      avgConfidence,
  ...
  getStats() {
    ...
    return {
      ...
      buffers: Array.from(this.buffer.entries()).map(([key, data]) => ({
        ...
        avgFill:
          data.length > 0
            ? this.calculateAverage(data.map((d) => d.fill))
            : null,
      })),
    };
  }
